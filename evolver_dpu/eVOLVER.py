...
IPP_SELECT_ADDRS = [40, 41, 42, 43, 44, 45, 46, 47]
    ...
    def on_broadcast(self, data):
        ...
        for param in od_cal['params']:
            self.save_data(data['data'].get(param, []), elapsed_time,
                    VIALS, param + '_raw')    
        #self.save_data(data['data']['lux'], elapsed_time, VIALS, 'lux')        
        ...
        if time_in_mins % 5 == 0 and time_in_mins is not 0 and elapsed_time * 60 - self.last_efflux * 60 > 2:
            run_efflux = True
...
#        self.stop_all_pumps(regular = False, ipp = False)
    ...
    def update_chemo(self, data, vials, bolus_in_s, period_config, immediate = False):
        ...
        for x in vials:
            ...
            if period_config[x] == 0:
                ...
                # efflux
                #MESSAGE['value'][x + 16] = '%.2f|%d' % (bolus_in_s[x] * 1,
                                                        #period_config[x])
    ...
    def custom_functions(self, data, vials, elapsed_time, last_efflux):
        ...
        if OPERATION_MODE == 'turbidostat':
            custom_script.turbidostat(self, data, vials, elapsed_time, last_efflux)
        elif OPERATION_MODE == 'chemostat':
            custom_script.chemostat(self, data, vials, elapsed_time, last_efflux)
        else:
